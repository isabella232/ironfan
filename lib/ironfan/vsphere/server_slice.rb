  ...
  module Vsphere
    class ServerSlice < Ironfan::ServerSlice
      ...
      def start(bootstrap = false)
        start_monitor_progess(name)
        task = cloud.fog_connection.start_cluster
        while !task.finished?
          sleep(monitor_interval)
          monitor_start_progress(name, task.get_progress, !bootstrap)
        end
        monitor_start_progress(name, task.get_progress, !bootstrap)
      ...
      def stop
        ...
        while !task.finished?
          sleep(monitor_interval)
          monitor_stop_progress(name, task.get_progress)
        end
        monitor_stop_progress(name, task.get_progress)
        update_fog_servers(task.get_progress.result.servers)
      ...
      def destroy
        ...
        while !task.finished?
          sleep(monitor_interval)
          monitor_delete_progress(name, task.get_progress)
        end
        monitor_delete_progress(name, task.get_progress)
        update_fog_servers(task.get_progress.result.servers)
      ...
      def create_servers(threaded = true)
        ...
        while !task.finished?
          ...
          monitor_launch_progress(name, task.get_progress)
        end
