...
module Ironfan
  ...
  class ServerSlice < Ironfan::DslObject
    ...
    # all roles generated by Ironfan for the cluster and facets
    def chef_roles
      [ cluster.chef_roles, facets.map(&:chef_roles) ].flatten.compact.uniq
    end

    # all items in the run_list of all cluster nodes
    def run_list
      [ cluster.run_list, facets.map(&:run_list), servers.map(&:run_list) ].flatten.compact.uniq
    end
